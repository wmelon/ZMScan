...
@implementation UIImage (ZMOrientation)
...
- (UIImage *)scaledImage:(CGPoint)point scaleFactor:(CGFloat)scaleFactor targetSize:(CGSize)size {
    ...
    CGFloat midX = point.x - scaledSize.width / 2.0;
    CGFloat midY = point.y - scaledSize.height / 2.0;
    
    CGRect newRect = CGRectMake(midX, midY, scaledSize.width, scaledSize.height);
    
    CGImageRef croppedImage = CGImageCreateWithImageInRect(cgImage, newRect);
    if (!croppedImage) return self;
...
+ (UIImage *)imageFromCIImage:(CIImage *)ciImage {
    ...
    if (cgImage) {
        ...
        resultImage = [UIImage imageWithCIImage:ciImage scale:1.0 orientation:(UIImageOrientationUp)];
    }
    /// 必须手动释放资源
